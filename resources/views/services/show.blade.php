...
<section style="display:flex;flex-direction:column;gap:16px;">
  <div style="border:1px solid #42648f;background:linear-gradient(135deg,#2e456a,#3d5d89);border-radius:16px;padding:16px;box-shadow:0 12px 24px rgba(20,34,58,.24);color:#fff;">
    <div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;align-items:flex-start;">
      <div>
        <p style="margin:0;font-size:11px;letter-spacing:.9px;text-transform:uppercase;color:#d8e8ff;font-weight:700;">Service Provider ID Details</p>
  ...
  <div style="display:grid;grid-template-columns:1fr;gap:14px;">
    <article style="border:1px solid #d7deec;border-radius:14px;background:#fff;padding:14px;">
      <h2 style="margin:0;color:#263f63;font-size:18px;font-weight:800;">Provider Details</h2>
      <div style="margin-top:10px;display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:10px;font-size:13px;color:#304a70;">
        <p style="margin:0;"><strong style="color:#1f3658;">Name:</strong> {{ $providerName }}</p>
        @if($providerContact !== '')
          <p style="margin:0;"><strong style="color:#1f3658;">Contact:</strong> {{ $providerContact }}</p>
        @endif
        @if($service->city)
          <p style="margin:0;"><strong style="color:#1f3658;">Base City:</strong> {{ $service->city->name }}</p>
        @endif
        @if($serviceCities->isNotEmpty())
          <p style="margin:0;"><strong style="color:#1f3658;">Service Cities:</strong> {{ $serviceCities->implode(', ') }}</p>
        @endif
      </div>

      @if(!empty($services))
        <h3 style="margin:14px 0 8px;color:#2f4e74;font-size:12px;letter-spacing:.8px;text-transform:uppercase;">Services Offered</h3>
    ...
    <aside style="border:1px solid #d7deec;border-radius:14px;background:#fff;padding:14px;">
      <h2 style="margin:0;color:#263f63;font-size:18px;font-weight:800;">Ratings</h2>
      <p style="margin:8px 0 0;color:#1f3658;font-size:34px;font-weight:800;line-height:1;">{{ number_format($reviewStats['average'], 1) }}</p>
      <p style="margin:6px 0 0;color:#4f6584;font-size:13px;">{{ $reviewStats['count'] }} review{{ $reviewStats['count'] === 1 ? '' : 's' }}</p>
    </aside>
  </div>

  <section style="border:1px solid #d7deec;border-radius:14px;background:#fff;padding:14px;">
    <h2 style="margin:0;color:#263f63;font-size:18px;font-weight:800;">Customer Reviews</h2>
    ...
    <div style="margin-top:12px;display:flex;flex-direction:column;gap:10px;">
      ...
      @empty
        <p style="margin:0;font-size:13px;color:#5d7394;">No reviews yet. Be the first to review this service.</p>
      @endforelse
    </div>
    ...
    @auth
      <form method="POST" action="{{ route('services.reviews.store', $service->public_page_slug) }}" class="service-review-form" style="margin-top:14px;display:flex;flex-direction:column;gap:10px;border:1px solid #d2e0f4;border-radius:12px;background:#f1f6ff;padding:12px;">
        @csrf
        <h3 style="margin:0;color:#1f3658;font-size:14px;font-weight:800;">{{ $myReview ? 'Update your review' : 'Add your review' }}</h3>

        <div>
          <label for="rating" style="display:block;font-size:12px;color:#304c75;font-weight:700;">Rating</label>
          ...
          <div id="rating" class="service-star-rating" style="margin-top:6px;">
            @for($rating = 5; $rating >= 1; $rating--)
              ...
              <label for="auth_rating_{{ $rating }}" title="{{ $rating }} star{{ $rating > 1 ? 's' : '' }}"></label>
            @endfor
          </div>
        ...
        <div>
          <label for="review" style="display:block;font-size:12px;color:#304c75;font-weight:700;">Review</label>
          <textarea id="review" name="review" rows="4" style="margin-top:4px;width:100%;border:1px solid #bfd0ea;border-radius:10px;padding:8px 10px;font-size:13px;" placeholder="Share your experience (optional)">{{ old('review', $myReview?->review) }}</textarea>
    ...
    @else
      <form method="POST" action="{{ route('services.reviews.store', $service->public_page_slug) }}" class="service-review-form" style="margin-top:14px;display:flex;flex-direction:column;gap:10px;border:1px solid #d2e0f4;border-radius:12px;background:#f1f6ff;padding:12px;">
        @csrf
        <h3 style="margin:0;color:#1f3658;font-size:14px;font-weight:800;">Add your review</h3>

        <div class="service-review-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(0,1fr));gap:10px;">
          <div>
            <label for="reviewer_name" style="display:block;font-size:12px;color:#304c75;font-weight:700;">Your name</label>
            <input id="reviewer_name" name="reviewer_name" type="text" value="{{ old('reviewer_name') }}" required style="margin-top:4px;width:100%;border:1px solid #bfd0ea;border-radius:10px;padding:8px 10px;font-size:13px;" placeholder="Enter your full name">
            @error('reviewer_name')<p style="margin:4px 0 0;color:#b91c1c;font-size:11px;">{{ $message }}</p>@enderror
          </div>
          <div>
            <label for="reviewer_email" style="display:block;font-size:12px;color:#304c75;font-weight:700;">Your email</label>
            <input id="reviewer_email" name="reviewer_email" type="email" value="{{ old('reviewer_email') }}" required style="margin-top:4px;width:100%;border:1px solid #bfd0ea;border-radius:10px;padding:8px 10px;font-size:13px;" placeholder="Enter your email">
        ...
        <div>
          ...
          <div id="rating" class="service-star-rating" style="margin-top:6px;">
            @for($rating = 5; $rating >= 1; $rating--)
              ...
              <label for="guest_rating_{{ $rating }}" title="{{ $rating }} star{{ $rating > 1 ? 's' : '' }}"></label>
            @endfor
          </div>
          @error('rating')<p style="margin:4px 0 0;color:#b91c1c;font-size:11px;">{{ $message }}</p>@enderror
        </div>
        ...
        <div>
          <label for="review" style="display:block;font-size:12px;color:#304c75;font-weight:700;">Review</label>
          <textarea id="review" name="review" rows="4" style="margin-top:4px;width:100%;border:1px solid #bfd0ea;border-radius:10px;padding:8px 10px;font-size:13px;" placeholder="Write your review (optional)">{{ old('review') }}</textarea>
          @error('review')<p style="margin:4px 0 0;color:#b91c1c;font-size:11px;">{{ $message }}</p>@enderror
        </div>

        <button type="submit" style="display:inline-flex;align-items:center;justify-content:center;border:none;border-radius:10px;background:#2f4f77;color:#fff;padding:9px 13px;font-size:13px;font-weight:800;cursor:pointer;">Submit Review</button>
      </form>
    @endauth
  </section>

  <div>
    <a href="{{ route('services.index') }}" style="font-size:13px;font-weight:700;color:#2e4f77;text-decoration:none;">← Back to all services</a>
  </div>
</section>
@endsection
