@php($logoUid = 'ao' . substr(md5(($className ?? '') . '|' . ($title ?? '') . '|' . uniqid('', true)), 0, 10))
<svg class="{{ $className ?? '' }}" viewBox="0 0 64 64" width="64" height="64" preserveAspectRatio="xMidYMid meet" role="img" aria-label="{{ $title ?? 'AajchaOffer' }}" xmlns="http://www.w3.org/2000/svg" style="display:block;max-width:100%;height:auto;">
  <defs>
    <linearGradient id="{{ $logoUid }}-bg" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="#C368CA"/>
      <stop offset="100%" stop-color="#F391A0"/>
    </linearGradient>
    <linearGradient id="{{ $logoUid }}-lens" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="#ffffff" stop-opacity="0.95"/>
      <stop offset="100%" stop-color="#ffe5f4" stop-opacity="0.92"/>
    </linearGradient>
  </defs>

  <rect x="2" y="2" width="60" height="60" rx="16" fill="url(#{{ $logoUid }}-bg)"/>
  <rect x="6" y="6" width="52" height="52" rx="13" fill="none" stroke="rgba(255,255,255,0.35)" stroke-width="1.4"/>

  <circle cx="30" cy="30" r="12" fill="none" stroke="url(#{{ $logoUid }}-lens)" stroke-width="4"/>
