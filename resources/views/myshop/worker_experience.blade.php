...
<div class="experience-letter" style="max-width:800px;margin:20px auto;padding:20px;border:1px solid #ddd;background:#fff">
  ...
  <p>
    This is to certify that <strong>{{ $worker->full_name }}</strong> has worked with <strong>{{ $shop->name }}</strong> as a <strong>{{ $worker->tags ? explode(',', $worker->tags)[0] : 'Shop Worker' }}</strong>.
  </p>
