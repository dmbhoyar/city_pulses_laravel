...
<div class="myshop-workers">
  ...
  <h3>Add Worker</h3>
  <form action="{{ route('create_worker_myservice') }}" method="POST">
    @csrf
    <div>
    ...
    <p>
      ...
      <a href="{{ route('myservice') }}" class="button">Back</a>
    </p>
  </form>
      ...
      <tbody>
        @foreach($workers as $w)
          <tr>
            ...
            <td>{{ $w->tags }}</td>
            <td>
              <form action="{{ route('update_worker_myservice') }}" method="POST">
                @csrf
                @method('PATCH')
                ...
                <div>
                  ...
                  <a href="{{ route('worker_experience_myservice', ['id' => $w->id]) }}" class="button" target="_blank">Experience Letter</a>
                </div>
              </form>
            </td>
          </tr>
