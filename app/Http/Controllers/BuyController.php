...
class BuyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $totalActive = Listing::query()->where('category', 'sell')->where('status', 'active')->count();
        $cityActive = $selectedCityId
            ? Listing::query()->where('category', 'sell')->where('status', 'active')->where('city_id', $selectedCityId)->count()
            : $totalActive;

        $selectedCityName = optional($cities->firstWhere('id', $selectedCityId))->name;

        return view('buy.index', compact('listings', 'cities', 'selectedCityId', 'selectedCityName', 'search', 'subcategory', 'totalActive', 'cityActive'));
    }

    public function show(Listing $listing)
    {
        if ($listing->category !== 'sell') {
            abort(404);
        }
        ...
        return view('buy.show', ['buy' => $listing]);
    }
    ...
    public function create()
    {
        ...
        return view('buy.create', compact('listing', 'cities', 'listingFee', 'paymentQrUrl', 'paymentBarcodeUrl'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('buy.index')->with('notice', 'Listing request submitted. Payment proof received and status is in review until superadmin approval.');
    }
    ...
    public function edit(Listing $listing)
    {
        ...
        return view('buy.edit', compact('listing', 'cities'));
    }
    ...
    private function ensureSellerRole(): void
    {
        ...
        if (!method_exists($user, 'isSeller') || !$user->isSeller()) {
            abort(403, 'Only seller role can post Buy & Sell listings.');
        }
    }
