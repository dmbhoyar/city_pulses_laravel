...
class MyserviceController extends Controller
{
    ...
    public function configureSave(Request $request)
    {
        ...
        $cfg['template_content'] = [
            'hero_badge'        => trim((string) $request->input('tc.hero_badge', 'Trusted Local Service')),
            'hero_title'        => trim((string) $request->input('tc.hero_title', 'Professional Service For Your Needs')),
            'hero_description'  => trim((string) $request->input('tc.hero_description', 'Fast, reliable, and affordable service from experienced professionals.')),
            'primary_cta'       => trim((string) $request->input('tc.primary_cta', 'Book Now')),
            'secondary_cta'     => trim((string) $request->input('tc.secondary_cta', 'Get Free Quote')),
            'services_label'    => trim((string) $request->input('tc.services_label', 'Our Services')),
            'services_title'    => trim((string) $request->input('tc.services_title', 'Services We Offer')),
            'services_subtitle' => trim((string) $request->input('tc.services_subtitle', 'Choose from our most popular services.')),
            'why_title'         => trim((string) $request->input('tc.why_title', 'Why Choose Us')),
            'why_subtitle'      => trim((string) $request->input('tc.why_subtitle', 'Quality work, transparent pricing, and quick response.')),
            'cta_title'         => trim((string) $request->input('tc.cta_title', 'Need Help Today?')),
